...
export default function EnrollmentPage() {
  ...
  const handleStatusChange = useCallback(
    async (row, nextStatus) => {
      ...
      setBusyId(row.id);
      try {
        await lmsApi.updateEnrollmentStatus({ enrollmentId: row.id, status: nextStatus });
        ...
        <Stack spacing={3}>
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              {isInstructorLike
                ? 'Review learner program applications, update approval status, and track pending requests.'
                : 'View your program enrollment applications and approval status.'}
            </Typography>
          </Box>
            ...
            <CardContent>
              <Stack spacing={2}>
                ...
                <Typography variant="body2" color="text.secondary" sx={{ maxWidth: 560 }}>
                  {isInstructorLike
                    ? 'Search matches learner name, email, phone, school held, program title, enrollment id, or status.'
                    : 'Programs you have applied to appear below with their current approval status.'}
                </Typography>
                {isInstructorLike ? (
                  <TextField
                    size="small"
